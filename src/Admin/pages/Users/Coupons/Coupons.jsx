  ...
  const getNoDataMessage = () => {
    ...
    return {
      generated: {
        ...
        description: "There are no coupons generated at the moment. New coupons will appear here once they are created."
      },
      claimed: {
        title: "No Claimed Coupons",
        description: "No coupons have been claimed yet. Claimed coupons will be displayed here once customers redeem them."
      }
    }[activeTab];
  };
              ...
              {loading ? (
                Array.from({ length: itemsPerPage }).map((_, index) => (
                  <tr key={index} className="border-t animate-pulse">
                    <td className="py-4"><div className="h-4 bg-gray-200 rounded w-6"></div></td>
                    ...
                    <td className="py-4"><div className="h-4 bg-gray-200 rounded w-40"></div></td>
                ...
                displayedCoupons.map((coupon, index) => (
                  <tr key={coupon.id} className="border-t">
                    ...
                    <td className="py-4">{formatDate(coupon.createdAt)}</td>
                    {activeTab === "claimed" && (
                      <td className="py-4">{formatDate(coupon.claimedAt)}</td>
                    )}
                    <td className="py-4">
                      <span 
                        className={`${
        ...
        {displayedCoupons.length > 0 && (
          <div className="mt-6 flex items-center justify-between">
            <div className="text-sm text-gray-500">
              Showing {startIndex + 1} - {Math.min(startIndex + itemsPerPage, filteredCoupons.length)} of{" "}
