...
const CashbackRequests = () => {
  ...
  const displayedCoupons = allCoupons.slice(
    startIndex,
        ...
        <div className="bg-white rounded-lg shadow-md border p-8">
          <div className="grid grid-cols-2 gap-y-8">
            ...
            <div>
              ...
              <div className="font-medium">{selectedCoupon.shopName}</div>
            </div>

            <div>
              <div className="text-gray-500 mb-1">Email</div>
              <div className="font-medium">{selectedCoupon.email}</div>
            </div>

            <div>
              <div className="text-gray-500 mb-1">Paid Amount</div>
              <div className="font-medium">{selectedCoupon.paidAmount}</div>
            </div>

            <div>
              <div className="text-gray-500 mb-1">Phone No.</div>
              <div className="font-medium">{selectedCoupon.phone}</div>
            </div>

            <div>
              <div className="text-gray-500 mb-1">Payment method</div>
              <div className="font-medium">{selectedCoupon.paymentMethod}</div>
              <div className="text-sm text-gray-500 mt-1">
                {selectedCoupon.paymentId}
              </div>
            </div>

            <div>
              <div className="text-gray-500 mb-1">Date</div>
              <div className="font-medium">{selectedCoupon.date}</div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded">
                  ...
                  <span>{selectedCoupon.invoice}</span>
                </div>
      ...
      <div className="bg-white rounded-lg border shadow-lg p-6">
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="text-sm text-gray-500">
            Showing {Math.min(itemsPerPage, allCoupons.length)} of{" "}
            {allCoupons.length}
          </div>
